...
def merge_dict(original, update):
    ...
    """
    if original.has_key('services'):
        original = original['services']
...
def get_compose_services(compose):
    ...
    for key in compose['services']:
        if not compose['services'][key].has_key('image'): continue
        services[key] = get_image_parts(compose['services'][key].get('image'))
    return services
...
def main(argv):
    ...
    eprint ("Matched these locators in the Buildreport file")
    eprint (matched_locators)
    withversions=merge_dict(withstaticversions, matched_locators)
    print(yaml.safe_dump({'version': '2', 'services': withversions},indent=2, default_flow_style=False))
